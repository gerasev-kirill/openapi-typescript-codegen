...
var partialBase = {"1":function(container,depth0,helpers,partials,data) {
  ...
  return ((stack1 = lookupProperty(helpers,"equals").call(depth0 != null ? depth0 : (container.nullContext || {}),lookupProperty(depth0,"base"),"binary",{"name":"equals","hash":{},"fn":container.program(1, data, 0),"inverse":container.program(4, data, 0),"data":data,"loc":{"start":{"line":1,"column":0},"end":{"line":8,"column":13}}})) != null ? stack1 : "");
},"useData":true};
...
function registerHandlebarTemplates(root) {
    ...
    const templates = {
        ...
        exports: {
            model: Handlebars__namespace.template(((_a = root.hbsFilesOverride) === null || _a === void 0 ? void 0 : _a.exportModel) || templateExportModel),
            schema: Handlebars__namespace.template(((_b = root.hbsFilesOverride) === null || _b === void 0 ? void 0 : _b.exportSchema) || templateExportSchema),
            service: Handlebars__namespace.template(((_c = root.hbsFilesOverride) === null || _c === void 0 ? void 0 : _c.exportService) || templateExportService),
        },
        core: {
 ...
 * @param useUnionTypes Use union types instead of enums
 */
async function writeClientModels(models, templates, outputPath, httpClient, useUnionTypes, additionalContext) {
    for (const model of models) {
        const file = path.resolve(outputPath, `${model.name}.ts`);
        const templateResult = templates.exports.model(Object.assign(Object.assign({}, model), { httpClient,
            useUnionTypes, additionalContext: additionalContext || {} }));
 ...
 */
async function writeClientSchemas(models, templates, outputPath, httpClient, useUnionTypes, additionalContext) {
    for (const model of models) {
        const file = path.resolve(outputPath, `$${model.name}.ts`);
 ...
 * @param postfix: Service name postfix
 */
async function writeClientServices(services, templates, outputPath, httpClient, useUnionTypes, useOptions, postfix, additionalContext) {
    for (const service of services) {
        ...
        const templateResult = templates.exports.service(Object.assign(Object.assign({}, service), { httpClient,
            ...
            useVersion,
            useOptions,
            postfix, additionalContext: additionalContext || {} }));
        await writeFile(file, format(templateResult));
    }
}
 ...
 * @param request: Path to custom request file
 */
async function writeClient(client, templates, output, httpClient, useOptions, useUnionTypes, exportCore, exportServices, exportModels, exportSchemas, postfix, request, additionalContext) {
    ...
    if (exportServices) {
        ...
        await writeClientServices(client.services, templates, outputPathServices, httpClient, useUnionTypes, useOptions, postfix, additionalContext);
    }
    if (exportSchemas) {
 ...
 */
async function generate({ input, output, httpClient = exports.HttpClient.FETCH, useOptions = false, useUnionTypes = false, exportCore = true, exportServices = true, exportModels = true, exportSchemas = false, postfix = 'Service', request, hbsFilesOverride, additionalContext, write = true, }) {
    ...
    switch (openApiVersion) {
        case OpenApiVersion.V2: {
            ...
            if (!write)
                break;
            await writeClient(clientFinal, templates, output, httpClient, useOptions, useUnionTypes, exportCore, exportServices, exportModels, exportSchemas, postfix, request, additionalContext);
            break;
        }
